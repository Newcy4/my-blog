...
import qs from 'querystring' // 处理地址栏的第三方库，和nodejs里面的query模块用法一样
  ...
  'data|10-20': [
    {
      'id|+1': 1,
      name: '分类@id',
      'articleCount|0-200': 10, //该分类下文章的数量
      'order|+1': 1
    }
  ]
})
  ...
  return Mock.mock({
    ...
    data: {
      ...
      [`rows|${query.limit}`]: [
        //当前页列表数据
        {
          id: '@guid', //全球唯一id，mock里面自带的函数，mock中@开头的都是mock里面的一个函数
          title: '@ctitle(1,50)', // 中文标题
          description: '@cparagraph(1, 10)', // 中文段落，1到10个句子
          category: {
            //所属分类
            'id|1-10': 1,
            name: '分类@id'
          ...
          'commentNumber|0-300': 1, // 评论数量
          'thumb|1': ['@image(300x250, @color, #fff, @natural)', null], //略缩图地址，这里模拟数据直接用mock随机生成的图片(尺寸，背景色，前景色，文字),缩略图有可能有有可能没有
